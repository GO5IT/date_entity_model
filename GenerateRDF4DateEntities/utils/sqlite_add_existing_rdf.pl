...
my $DEFAULTDB="sqlite_year.db";
my $DEFAULTMODEL="year_rdf";
...
our ($opt_h, $opt_d, $opt_i,$opt_o,$opt_l,$opt_u,$opt_t,$opt_L, $opt_S, $opt_D, $opt_m);
getopts('hdi:o:l:u:tL:S:D:m:');
...
experiment with a SQLite - based version ...
...
-D <DATABASEFILE> 
		Name of the SQLite Databasefile
            DEFAULT: $DEFAULTDB

-m <MODELNAME> ??? Not sure if we need one
...
my $dbname = $opt_D ? $opt_D : $DEFAULTDB;
my $modelname = $opt_m ? $opt_m : $DEFAULTMODEL;
...
##my $dbstore = RDF::Trine::Store::DBI->new( $modelname, $dbh );
## 

my $dbstore = RDF::Trine::Store->new({
                                     storetype => 'DBI',
                                     name      => "$modelname",
                                     dsn       => "dbi:SQLite:dbname=$dbname",
                                     username  => '',
...
# Finally, wrap the Store object into a Model, and use it to access your data
my $model = RDF::Trine::Model->new($dbstore);
